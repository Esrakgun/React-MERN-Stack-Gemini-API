...
const StyleSelector = ({
  ...
  const styleDesciptions: Record<ThumbnailStyle, string> = {
    "Bold & Graphic": "High contrast, bold typography, striking visuals",
    Minimalist: "Clean,simple, loost of white space",
  ...
  const styleIcons: Record<ThumbnailStyle, React.ReactNode> = {
    ...
    Illustrated: <PenToolIcon className="size-4" />,
    ...
    <div className="relative space-y-3 dark">
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-400">{styleDesciptions[value]}</p>
        </div>
        <ChevronDownIcon
          className={[
          ...
          {thumbnailStyles.map((style) => (
            ...
            >
              ...
              <div>
                        ...
                        <p className="text-xs text-zinc-400">{styleDesciptions[style]}</p>
              </div>
            </button>
          ))}
        </div>
